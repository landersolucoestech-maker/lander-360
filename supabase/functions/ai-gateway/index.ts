...
async function callAI(
  ...
  console.log(`[AI Gateway] Provider: ${provider}, Model: ${selectedModel}, Stream: ${stream}`);
  ...
  if (!response.ok) {
    ...
    console.error(`[AI Gateway] ${provider} error:`, response.status, errorText);
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[AI Gateway] Task: ${task || 'general'}, Selected: ${finalProvider}`);
