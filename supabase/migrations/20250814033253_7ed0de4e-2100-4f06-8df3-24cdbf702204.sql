...
-- Admins can do everything, regular members can only view basic info (no email/tax_id)
...
WITH CHECK (false); -- Prevent any modifications
...
CREATE POLICY "contracts_member_view_own"
ON public.contracts
FOR SELECT
TO authenticated
USING (
  is_org_member(org_id) AND (
    artist_id IN (
      SELECT id FROM public.artists WHERE org_id = contracts.org_id
    ) OR
    contributor_id IN (
      SELECT id FROM public.contributors WHERE org_id = contracts.org_id
    )
  )
);
...
-- Payments table - sensitive financial transactions
...
-- Sales reports and lines - financial data
